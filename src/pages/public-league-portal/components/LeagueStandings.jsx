...
// Helper component for rendering a single stage's standings table
const StageStandingsTable = ({ stageName, standings, selectedSeason, leagues, selectedLeagueId, onLeagueChange, isLoading, getFormIndicator, getPositionChange }) => {
    ...
    <div className="bg-card rounded-lg border border-border card-shadow">
      <div className="p-6 border-b border-border">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h2 className="text-xl font-semibold text-foreground">{stageName} Standings</h2>
      ...
      {isLoading ? (
        ...
        <div className="p-12 text-center">
          ...
          <p className="text-muted-foreground">No standings available for {stageName}.</p>
        </div>
      ) : (
        <>
          {/* Desktop Table View */}
...
const LeagueStandings = ({ standings, selectedSeason, leagues, selectedLeagueId, onLeagueChange, isLoading }) => {
      ...
      <div className="space-y-8">
        {stagesWithData.map((stage) => (
          <StageStandingsTable
            ...
            standings={standings[stage]}
            selectedSeason={selectedSeason}
            leagues={leagues}
            selectedLeagueId={selectedLeagueId}
            onLeagueChange={onLeagueChange}
            isLoading={isLoading}
            getFormIndicator={getFormIndicator}
  ...
  // Legacy: render single table for flat array (fallback for backward compatibility)
  const selectedLeague = leagues?.find(l => l?.id === selectedLeagueId);
  const leagueOptions = leagues?.map(league => ({
    value: league?.id,
    label: `${league?.name} (${league?.season || 'N/A'})`
    ...
    <div className="bg-card rounded-lg border border-border card-shadow">
      ...
      {isLoading ? (
        ...
        <div className="p-12 text-center">
          ...
          <p className="text-muted-foreground">Standings are now organized by stage. Please refresh the page.</p>
        </div>
      )}
    </div>
  );
